...
export default function TutorialPage({ source, frontMatter }) {
    return (
...
export async function getStaticPaths(){
    ...
    const files = fs.readdirSync(path.join('data/tutorial'));
...
export async function getStaticProps({ params: { slug } }){
    const metaMarkdownFile = fs.readFileSync(path.join('data/tutorial', slug + '.mdx'), 'utf-8');
